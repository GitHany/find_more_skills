...
```
扫描项目 → 确认专家列表 → 并发派遣 Agent → 聚合去重 → 生成报告文档
```
...
- **默认**：派遣全部 8 位专家
- **用户指定**：用户可指定需要的专家（如 "只看安全和性能"）
- **项目自适应**：根据项目类型自动调整
  - 纯后端项目 → 移除 Pixel（UX）
  - 无 CI/CD → Forge 侧重"建议搭建"
  - 库/SDK 项目 → 增加 Spark 的 API 设计维度
...
当用户选择"快速模式"时，只派遣 3 位核心专家：

| 专家 | 代号 | 理由 |
|------|------|------|
| 架构师 | Atlas | 全局视角，发现结构性问题 |
| 代码质量专家 | Craft | 代码层面问题最多 |
| 安全专家 | Shield | 安全问题不可忽视 |

快速模式的报告结构相同，但分析深度降低（每个专家发现数上限 15 条）。
